...
public class BluetoothPlugin2 extends CordovaPlugin {
	...
	public BluetoothPlugin2() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
			try {
				...
				BluetoothPlugin2.ACTION_UUID = (String) actionUUID.get(null);
				Log.d("BluetoothPlugin2", "actionUUID: " + actionUUID.getName() + " / " + actionUUID.get(null));
				Field extraUUID = BluetoothDevice.class.getDeclaredField("EXTRA_UUID");
				BluetoothPlugin2.EXTRA_UUID = (String) extraUUID.get(null);
				Log.d("BluetoothPlugin2", "extraUUID: " + extraUUID.getName() + " / " + extraUUID.get(null));
			}
			catch( Exception e ) {
				logErr( e.getMessage() );
			}
	...
	public boolean execute(String action, JSONArray args,
				...
				public void run() {
					...
					try {
						...
						*/
						...
						System.out.println("BluetoothPlugin2.ACTION_CONNECT  address=" + address + ", uuid=" + uuid + ", secure=" + secure);
						BluetoothDevice device = BluetoothAdapter.getDefaultAdapter().getRemoteDevice(address); //"74:45:8A:B2:4E:9A"); 
		...
		else if ( ACTION_LISTEN.equals(action) )
		{
			...
			final String uuid = args.getString(1);
			final boolean secure = args.getBoolean(2);
			System.out.println("BluetoothPlugin2.ACTION_LISTEN  name=" + name + ", uuid=" + uuid + ", secure=" + secure);
	...
	private class BPBroadcastReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			else if (BluetoothPlugin2.ACTION_UUID.equals(action)) {
				m_gotUUIDs = new JSONArray();
				Parcelable[] parcelUuids = 
						intent.getParcelableArrayExtra(BluetoothPlugin2.EXTRA_UUID);
				if (parcelUuids != null) {
	...
	private void logDbg(String msg) {
		Log.d("BluetoothPlugin2", msg);
	}

	
	
	private void logErr(String msg) {
		Log.e("BluetoothPlugin2", msg);
	}
