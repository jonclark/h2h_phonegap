...
var listeningUUID = "00001101-0000-1000-8000-00805f9b34fb"; //"e7babb53-0160-4192-a2b6-2ccb1e9aae67"
...
function onload() {
  ...
  disableBluetoothRelatedInputs(true); //JWC - changed from true to false for debugging purposes.
...
function toggleListen(item) {
  ...
  console.log('In toggleListen()');
  if(item.value == "Listen") {
    //item.disabled = true;  //JWC
	  console.log('In toggleListen() 1');
    if (bluetoothPugin) {
  	  console.log('In toggleListen() 2');
      bluetoothPugin.listen(
        function(sockId){
      	  console.log('In toggleListen() 3');
          item.disabled = false;
          item.value = "Cancel";
          document.getElementById("listen-textarea").value = "Connected socket:" + sockId + "\n";
          bluetoothPugin.read(
      			function( p_data ) {
      			  console.log('In toggleListen() 4 reading!');
      				var str = "";
      				for ( var i = 0; i < p_data.length; i++) {
						str += String.fromCharCode(p_data[i]);
					}
      				document.getElementById("listen-textarea").value += "Data : >" + str + "<\n";
      			  console.log('In toggleListen() 5 reading done');
      			},
      			function( error ) {
      				document.getElementById("listen-textarea").value += "Error : >" + error + "<\n";
      			  console.log('In toggleListen() 6');
      			}, 
      			sockId);
        },
        function(error){
      	  console.log('In toggleListen() 7');
         item.disabled = false;
          item.value = "Listen";
